...
uint16_t ieee154e_getASNFirstBytes(void);
// synchronization
   ...
   if (ieee154e_vars.isSync==FALSE) {
      ...
      switch (ieee154e_vars.state) {
         ...
         case S_TXACK:
            ...
            break;
         case S_RXDATA_JAM:
        	 //it should not happen (radio is off), but discard the event without logging
...
//   						 (errorparameter_t)98);
   ...
   */
   do { // this "loop" is only executed once
    ...
    //   openserial_printError(COMPONENT_IEEE802154E,ERR_INVALIDPACKETFROMRADIO,
    //                         (errorparameter_t)1492,
    //                         (errorparameter_t)989);
...
//      //SoC code
//      if (ieee154e_vars.dataReceived->length == DECOY_PKT_LEN ||
//			ieee154e_vars.dataReceived->length == INFO_PKT_LEN ){
//		break;
//	  }
...
port_INLINE void activity_ti1ORri1(void) {
    cellType_t  cellType;
    cellType_t  temp_cellType; //antiJam, used to verify it is not a serial slot
...
//    if (ieee154e_vars.asn.bytes0and1 == ANTIJAM_STOP_TIME){
//    	leds_debug_off();
//    	openserial_printInfo(COMPONENT_IEEE802154E,ERR_OK,
//							 (errorparameter_t)ieee154e_vars.asn.bytes0and1,
//							 (errorparameter_t)103);
//    	if(!idmanager_getIsDAGroot()) idmanager_init();
//    	ieee154e_vars.soc_finished = 1;
//    }
...
port_INLINE void activity_ti2(void) {
    ...
    // openserial_printInfo(COMPONENT_IEEE802154E,ERR_WRONG_CELLTYPE,
    //                            (errorparameter_t)ieee154e_vars.freq,
    //                            (errorparameter_t)1234);
    ...
    // configure the radio for that frequency
    radio_setFrequency(ieee154e_vars.freq);
    radio_setTxPower(ieee154e_vars.localCopyForTransmission.l1_txPower);
    ...
    /*openserial_printInfo(COMPONENT_IEEE802154E,ERR_WRONG_CELLTYPE,
                               (errorparameter_t) ieee154e_vars.localCopyForTransmission.payload,
                               (errorparameter_t) ieee154e_vars.localCopyForTransmission.length);
                               */
    ...
    // if (ieee154e_vars.localCopyForTransmission.creator == COMPONENT_KMP && idmanager_getIsDAGroot()==FALSE){
    //            	openserial_printInfo(COMPONENT_KMP,ERR_OK,
    //        							(errorparameter_t)0,
    //        							(errorparameter_t)5011);
...
port_INLINE void activity_ti4(PORT_TIMER_WIDTH capturedTime) {
    // change state
    //ieee154e_vars.txStartDataTime = capturedTime;
    changeState(S_TXDATA);
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
    // cancel tt3activity_ti4
...
port_INLINE void activity_ti5(PORT_TIMER_WIDTH capturedTime) {
    //ieee154e_vars.txEndDataTime = capturedTime;
    bool listenForAck;
...
//    if (ieee154e_vars.dataToSend->SoC == 1){
//        	openserial_printError(COMPONENT_IEEE802154E,ERR_WDACKDURATION_OVERFLOWS,
//        			                         (errorparameter_t)capturedTime,
//        			                         (errorparameter_t)ieee154e_vars.dataToSend->length);
//
//        }
...
port_INLINE void activity_ti8(PORT_TIMER_WIDTH capturedTime) {
    //ieee154e_vars.txStartAckTime = capturedTime;
    // change state
    changeState(S_RXACK);
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
    // cancel tt7
...
port_INLINE void activity_ti9(PORT_TIMER_WIDTH capturedTime) {
    //ieee154e_vars.txEndAckTime = capturedTime;
    ieee802154_header_iht     ieee802514_header;
    ...
    //openserial_printError(COMPONENT_SECURITY,ERR_SECURITY,ieee154e_getASNFirstBytes(),7777);
    ...
    */
    ...
    endSlot();
    //openserial_printError(COMPONENT_SECURITY, ERR_SECURITY, ieee154e_vars.txStartDataTime, ieee154e_vars.txEndDataTime);
    //openserial_printError(COMPONENT_SECURITY, ERR_SECURITY, ieee154e_vars.txStartAckTime, ieee154e_vars.txEndAckTime);
}
...
port_INLINE void activity_ri4(PORT_TIMER_WIDTH capturedTime) {
    //ieee154e_vars.rxStartDataTime = capturedTime;
...
port_INLINE void activity_ri5(PORT_TIMER_WIDTH capturedTime) {
    //ieee154e_vars.rxEndDataTime = capturedTime;
    ieee802154_header_iht       ieee802514_header;
    uint16_t                    lenIE=0;
    ...
    */
    do { // this "loop" is only executed once
        ...
        // PRINT PAAYLOAD LENGTH
        //if(idmanager_getIsDAGroot() == TRUE){
        // openserial_printInfo(COMPONENT_KMP,ERR_OK,
        //                        (errorparameter_t) 2000,
...
port_INLINE void activity_ri6(void) {
    ...
    // space for 2-byte CRC
   packetfunctions_reserveFooterSize(ieee154e_vars.ackToSend,2);
   //openserial_printError(COMPONENT_SECURITY,ERR_SECURITY, (errorparameter_t) ieee154e_vars.ackToSend->length, 1492);
...
port_INLINE void activity_ri8(PORT_TIMER_WIDTH capturedTime) {
    //ieee154e_vars.rxStartAckTime = capturedTime;
    // change state
    changeState(S_TXACK);
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
    // cancel rt7
...
port_INLINE void activity_ri9(PORT_TIMER_WIDTH capturedTime) {
    //ieee154e_vars.rxEndAckTime = capturedTime;
   // change state
   ...
   endSlot();
   //openserial_printError(COMPONENT_SECURITY, ERR_SECURITY, ieee154e_vars.rxStartDataTime, ieee154e_vars.rxEndDataTime);
   //openserial_printError(COMPONENT_SECURITY, ERR_SECURITY, ieee154e_vars.rxStartAckTime, ieee154e_vars.rxEndAckTime);
}
...
uint16_t ieee154e_getASNFirstBytes(void){
	return ieee154e_vars.asn.bytes0and1;
}
